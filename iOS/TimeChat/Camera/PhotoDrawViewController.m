...
- (void)viewDidLoad
{
    ...
    filename = @"draw_t1.png";//[NSString stringWithFormat:@"draw_pen_selected%@", fileSufix];
    ...
    sizePhotoImageView.origin.x = 0;
    sizePhotoImageView.origin.y = 0;
    sizePhotoImageView.size.height = self.view.frame.size.height;
    photoImageView = [[UIImageView alloc] initWithFrame:sizePhotoImageView];
    [photoImageView setImage:photoImage];
    [self.view addSubview:photoImageView];
    
    drawView = [[UIImageView alloc] initWithFrame:sizePhotoImageView];
    ...
    [pencilButton addTarget:self action:@selector(clickTextButton) forControlEvents:UIControlEventTouchUpInside]; ////@selector(clickPencilButton)
    ...
    sizeColorButton.size.width = 70 * scale;
    sizeColorButton.size.height = 70 * scale;
    sizeColorButton.origin.y = 0;
...
- (void)clickSaveButton {    
    if ([drawImageArray count] == 0 && [textAry count] == 0 ) {
        ...
        [photoImageView.image drawInRect:CGRectMake(0,0,newSize.width,newSize.height)];
        ...
        for (int i = 0; i < [textAry count]; i++) {
            ...
            NSDictionary *attr = @{NSFontAttributeName:font , NSForegroundColorAttributeName:InputTextField.textColor, NSParagraphStyleAttributeName:style};
            [str drawInRect:TextField.frame withAttributes: attr];
        }
        
        UIImage *newImage = UIGraphicsGetImageFromCurrentImageContext();
        UIGraphicsEndImageContext();
        [delegate savePhoto:newImage];
    }
...
- (void)touchesBegan:(NSSet  *)touches withEvent:(UIEvent *)event {
    ...
    }else if(instrument && !pickerColor) {
        mouseSwiped = NO;
        lastPoint = [touch locationInView:photoImageView];
    } else if (is_text && !pickerColor){
        lastPoint = [touch locationInView:photoImageView];
        if (lastPoint.y > colorSizePanelImageView.frame.origin.y && lastPoint.y < colorSizePanelImageView.frame.origin.y + colorButton.frame.size.height) {
    ...
    if(lastPoint.x > 0 && lastPoint.y > 0 && !pickerColor) {
        ...
        if([self enterPoint:currentPoint]) {
            ...
            drawColor = [self colorFromImage:colorPickerImageView.image
                              sampledAtPoint: CGPointMake(point.x/colorPickerImageView.frame.size.width,
