...
- (void)createButtonsWithText:(NSString *)text atPoint:(CGPoint)point
{
	...
	for (NSString *expression in expressions)
	{
		...
		while (match = [enumerator nextObject])
		{
			CGSize matchSize = [match sizeWithAttributes:@{NSFontAttributeName:font}];
            ...
            CGSize measureSize = [measureText sizeWithAttributes:@{NSFontAttributeName:font}];
...
- (void)createButtons
{
	...
	while (NSMaxRange(scanRange) < textLength)
	{
		...
		else
		{
            ...
            CGSize currentSize = [scanText sizeWithAttributes:@{NSFontAttributeName:font}];
