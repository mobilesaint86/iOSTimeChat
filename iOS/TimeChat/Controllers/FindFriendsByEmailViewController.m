//
//  FindFriendsByEmailViewController.m
//  TimeChat1
//
...
@interface FindFriendsByEmailViewController () {
...
@implementation FindFriendsByEmailViewController
...
- (void)viewDidLoad
{
    ...
    filename=[NSString stringWithFormat:@"find_by_email%@", fileSufix];
    image = [UIImage imageNamed:filename];
    ...
    UIImageView *emailImageView = [[UIImageView alloc] initWithFrame:size];
    ...
    size.size.height = image.size.height * scale;
    size.size.width = image.size.width * scale;
    size.origin.y  = emailImageView.frame.origin.y + emailImageView.frame.size.height + heightSpace;
    size.origin.x = (screenWidth - size.size.width) / 2;
    ...
    [emailTextField setTextColor:lightTextColor];
...
- (void)clickSendInvite {
    ...
    if(![emailTextField.text isEqualToString:@""]) {
        ...
        NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@", [UserDataSingleton sharedSingleton].serverURL, @"friends/add_friend"]];
        ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
        request.userInfo = [NSDictionary dictionaryWithObject:@"add_friend" forKey:@"type"];
        ...
        [self createAlertView:@"Please enter email"];
    }
}
...
{
    if ([[request.userInfo objectForKey:@"type"] isEqualToString:@"add_friend"]) {
        ...
        if(!error) {
            ...
            if([[message objectForKey:@"type"] isEqualToString:@"success"]) {
                    ...
                    if ([[contactDictionary objectForKey:@"code"] integerValue] == USER_REGISTERED) {
                    ...
                    [self createAlertView:@"Can not find this user."];//[message objectForKey:@"value"]
            }
        } else {
