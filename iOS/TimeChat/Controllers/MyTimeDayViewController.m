...
@interface MyTimeDayViewController () {
    ...
    UIButton *shareButton,*likeButton,*deleteButton,*commentButton, *cancelButton, *commentTextFieldButton, *recordButton, *newDeleteButton;
    NSArray *mediasArray;
    ...
    CGRect sizeDeleteButton;
}
@end

@implementation MyTimeDayViewController
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if([UserDataSingleton sharedSingleton].mainMediaID){
        ...
        [self photoView:[UserDataSingleton sharedSingleton].mainMediaID];
    }else{
...
- (void)create {
    ...
    InputTextColor = [UserDataSingleton sharedSingleton].InputTextColor;
    ...
    // Title label
    widthSpace = 345 * scale;
    size.size = [@"09:24am  " sizeWithAttributes:@{NSFontAttributeName:font1}];
    size.origin.x = widthSpace;
    ...
    commentFormViewHolder = [[UIView alloc] initWithFrame:CGRectMake(0, statusBarHeight, self.view.frame.size.width,self.view.frame.size.height - statusBarHeight)];
...
- (void)zoomImage:(id)sender{
    ...
    NSError *error = nil;
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"media_id==%@", zoomMediaID];
    ...
    if ([mediasArray count]>0){
        ...
        if (mediadata.video_data!=nil || mediadata.data!=nil)
            [self photoView:zoomMediaID];
    }else{
    ...
    if ([mediasArray count] == 0){
        ...
        for (int i = 0; i < 24; i++) {
            ...
            for (int k = 0; k < [mediasArray count]; k++) {
                ...
                if ([media.user_id isEqualToString: [UserDataSingleton sharedSingleton].idUser]) {
...
- (void)hourView:(int)selectedHour{
    ...
    [fetchRequest setEntity:entity];
    NSError *error = nil;
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"user_id==%@",
    ...
    if([mediasArray count]>0) {
        ...
        if ([photoNum isEqualToString:media.media_id] && mediadata.data!=nil) {
            ...
            if ([UserDataSingleton sharedSingleton].mainMediaID){
                ...
                titleLabel.text = titleTime;
            }
            
            photoImage=[UIImage imageWithData:mediadata.data];
            
            photoImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, cellSize, cellSize / photoImage.size.width * photoImage.size.height)];
            ...
            sizeLikeButton.origin.x    = widthSpace;
            likeButton = [[UIButton alloc] initWithFrame:sizeLikeButton];
            [likeButton setBackgroundColor:[UIColor clearColor]];
            [likeButton setBackgroundImage:[UIImage imageNamed:filename] forState:UIControlStateNormal];
            ...
            // delete
            widthSpace = 660 * scale;
            heightSpace = 10 * scale;
            filename = [NSString stringWithFormat:@"comments_delete%@", fileSufix];
            UIImage *deleteImage = [UIImage imageNamed:filename];
            
            sizeDeleteButton.size.height = deleteImage.size.height * scale;
            sizeDeleteButton.size.width  = deleteImage.size.width * scale;
            sizeDeleteButton.origin.y    = infoBackgroundView.frame.origin.y;
            sizeDeleteButton.origin.x    = widthSpace;
            deleteButton = [[UIButton alloc] initWithFrame:sizeDeleteButton];
            [deleteButton setBackgroundColor:[UIColor clearColor]];
            [deleteButton setBackgroundImage:[UIImage imageNamed:filename] forState:UIControlStateNormal];
            filename = [NSString stringWithFormat:@"comments_delete_down%@", fileSufix];
            [deleteButton setBackgroundImage:[UIImage imageNamed:filename] forState:UIControlStateHighlighted];
            
            [deleteButton addTarget:self action:@selector(deleteButtonPress:) forControlEvents:UIControlEventTouchUpInside];
            [photoFormViewHolder addSubview:deleteButton];
...
- (void)sendDelete {
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@", [UserDataSingleton sharedSingleton].serverURL, @"medias/destroy_media"]];
    ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
    request.userInfo = [NSDictionary dictionaryWithObject:@"destroy_media" forKey:@"type"];
    [request setTimeOutSeconds:30.f];
    [request setPostValue:zoomMediaID forKey:@"media_id"];
    [request setPostValue:[UserDataSingleton sharedSingleton].session forKey:@"token"];
    [request setDelegate:self];
...
-(void)shareButtonPress:(UIButton*)button {
    ...
    if (![zoomMediaID isEqualToString:@"0"]) {
        ...
        subVC.typeMedia = [NSString stringWithFormat:@"%li", (long)button.tag];
        [self presentViewController:subVC animated:YES completion:nil];
    }
}
...
-(void)deleteButtonPress:(UIButton*)button {
    ...
    if (![zoomMediaID isEqualToString:@"0"]) {
        UIActionSheet *menu = [[UIActionSheet alloc]
                               ...
                               otherButtonTitles:@"Delete this Photo", nil];
...
-(void)menuButtonPress:(UIButton*)button {
    NSString *filename = [NSString stringWithFormat:@"forgot_button%@", fileSufix];
    // delete
    widthSpace = 660 * scale;
    heightSpace = 10 * scale;
    filename = [NSString stringWithFormat:@"forgot_button%@", fileSufix];
    UIImage *deleteImage = [UIImage imageNamed:filename];
    CGRect sizeNewDeleteButton;
    sizeNewDeleteButton.size.height = deleteImage.size.height * scale;
    sizeNewDeleteButton.size.width  = deleteImage.size.width * scale;
    sizeNewDeleteButton.origin.y    = sizeDeleteButton.origin.y - sizeNewDeleteButton.size.height;
    sizeNewDeleteButton.origin.x    = widthSpace;
    newDeleteButton = [[UIButton alloc] initWithFrame:sizeDeleteButton];
    [newDeleteButton setBackgroundColor:[UIColor clearColor]];
    [newDeleteButton setTitle:@"Delete" forState:UIControlStateNormal];
    [newDeleteButton setBackgroundImage:[UIImage imageNamed:filename] forState:UIControlStateNormal];
    
    [newDeleteButton addTarget:self action:@selector(deleteButtonPress:) forControlEvents:UIControlEventTouchUpInside];
    [photoFormViewHolder addSubview:newDeleteButton];
    ...
    if(!error) {
            ...
            if ([[message objectForKey:@"code"] intValue] == SUCCESS_QUERY){
                if ([[request.userInfo objectForKey:@"type"] isEqualToString:@"send_like"]) {
                        ...
                        [UserDataSingleton sharedSingleton].changed = true;
                        [self func:data];
                } else if ([[request.userInfo objectForKey:@"type"] isEqualToString:@"destroy_media"]) {
                        NSFetchRequest *fetchRequest = [[NSFetchRequest alloc] init];
                        NSEntityDescription *entity = [NSEntityDescription entityForName:@"Medias" inManagedObjectContext:[CoreDataManager sharedManager].managedObjectContext];
                        [fetchRequest setEntity:entity];
